...
WI 범위가 필요한 스킬만 `state.plane_context.focused_work_item_id` (또는 `work_item_ids[0]`)를 기준으로 동작한다.
예: `omk focus`, `omk subtask`, `omk done`, `omk switch-task`, `omk note`, `omk decision`, `omk handoff`, `omk comments`, `omk history`, `omk sprint`, `omk open`.
