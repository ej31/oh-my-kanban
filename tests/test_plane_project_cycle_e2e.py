...
def ctx() -> CliContext:
    ...
    context._client.users.get_me.return_value.id = "user-id-123"
    return context
...
class TestProjectCreate:
    def test_create_minimal(self, runner, ctx):
        ...
        created = MagicMock()
        ctx.client.projects.create.return_value = created
...
class TestCycleList:
    ...
    def test_list_without_project_fails(self, runner):
        ...
        result = runner.invoke(cycle, ["list"], obj=no_proj_ctx)
        assert result.exit_code != 0
...
class TestCycleCreate:
    def test_create_with_dates(self, runner, ctx):
        ...
        created = MagicMock()
        ctx.client.cycles.create.return_value = created
        result = runner.invoke(
            cycle,
            [
                "create",
...
class TestStateUpdate:
    def test_update_state(self, runner, ctx):
        ...
        result = runner.invoke(
            state, ["update", "st-001", "--name", "Done"], obj=ctx
        )
        assert result.exit_code == 0, result.output
