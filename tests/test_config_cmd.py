...
def test_config_is_top_level_command(runner):
    """config는 최상위(top-level) 커맨드여야 한다 (plane/github 아래가 아님)."""
    result = runner.invoke(cli, ["config", "--help"])
...
def test_config_accessible_without_provider_prefix(runner):
    ...
    assert result.exit_code == 0
    # plane이나 github 하위가 아닌 최상위에서 접근 가능
...
def test_config_is_not_subcommand_of_github(runner):
    """omk github config는 실패해야 한다 (config는 github 하위 커맨드가 아님)."""
    result = runner.invoke(cli, ["github", "config", "--help"])

    # github 하위에 config가 없으므로 에러 종료여야 함
    assert result.exit_code != 0
