...
class TestModuleList:
    ...
    def test_list_without_project_fails(self, runner, no_project_ctx):
        result = runner.invoke(module, ["list"], obj=no_project_ctx)
        assert result.exit_code != 0
...
class TestModuleItems:
    def test_items_returns_list(self, runner, ctx):
        ctx.client.modules.list_work_items.return_value = _paginated([MagicMock()])
...
class TestEpicList:
    ...
    def test_list_without_project_fails(self, runner, no_project_ctx):
        result = runner.invoke(epic, ["list"], obj=no_project_ctx)
        assert result.exit_code != 0
...
class TestUserMe:
    ...
    def test_user_me_without_api_key_fails(self, runner, no_api_key_ctx):
        result = runner.invoke(user, ["me"], obj=no_api_key_ctx)
        assert result.exit_code != 0
...
class TestWorkspaceMembers:
    ...
    def test_members_without_workspace_fails(self, runner, no_workspace_ctx):
