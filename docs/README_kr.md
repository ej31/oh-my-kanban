...
- provider-local 모듈만 canonical 구현 경로로 사용
- 새 provider-specific 코드는 `providers/<name>/` 아래에 추가

## Agent Workflow

AI agent가 CLI를 안전하게 쓰려면 아래 순서로 정보를 읽는 것을 권장합니다.

1. `omk config --help`
2. `omk config show --profile PROFILE`
3. `omk plane --help` 또는 `omk linear --help`
4. 목표 서브커맨드의 `--help`

숨은 전제는 다음과 같습니다.

- Plane의 workspace/project context는 root가 아니라 `omk plane` 아래에 있다
- Plane 값은 group option, environment variable, config에서 올 수 있다
- Linear 자격 증명은 environment variable 또는 config에서 온다
- 팀 범위 Linear 명령은 `--team` → `LINEAR_TEAM_ID` → config 순서로 fallback한다
