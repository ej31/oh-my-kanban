...
- 잘못된 `upload_level` 값은 자동으로 `none`으로 대체됩니다 (안전 기본값 — 업로드 없음).
- `upload_level` 변경은 세션 종료 시 설정을 다시 읽으므로 해당 세션의 업로드에 즉시 반영될 수 있습니다. 예측 가능한 동작을 원한다면 세션 도중에 `upload_level`을 변경하지 마세요.
